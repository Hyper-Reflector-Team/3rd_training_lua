...
function load_spectator_settings()
    ...
    if _spectator_settings == nil then _spectator_settings = {} end

    if _spectator_settings.version == spectator_settings.version then
        for _key, _value in pairs(_spectator_settings) do if type(_value) == "number" then spectator_settings[_key] = _value end end
    end
end

-- !settings

display_mode = {"none", "P1+P2", "P1", "P2"}
...
settings_menu = make_menu(71, 40, 312, 105, -- screen size 383,223
{
    ...
    list_menu_item("Display Hitboxes", spectator_settings, "display_hitboxes", display_mode), list_menu_item("Display Gauges", spectator_settings, "display_gauges", display_mode),
    checkbox_menu_item("Display Distances", spectator_settings, "display_distances")
}, save_spectator_settings, false)
-- !menu

function on_start() load_spectator_settings() end

function on_load_state() end
...
function before_frame()
    ...
    if developer_mode then
        local _write_game_vars_settings = {infinite_time = true}
...
function on_gui()
    ...
    local _input = {down = hotkey3_pressed, up = hotkey2_pressed, left = false, right = hotkey4_pressed, validate = false, reset = false, cancel = false}
    ...
    if spectator_settings.display_input_history == 2 or spectator_settings.display_input_history == 4 then
        module_input_history.input_history_draw(module_input_history.input_history[2], util_draw.screen_width - 4, 49, true)
    end
    ...
    if is_in_match then
        ...
        if spectator_settings.display_hitboxes == 2 or spectator_settings.display_hitboxes == 3 then
            ...
            -- projectiles
            for _id, _obj in pairs(projectiles) do if _obj.emitter_id == 1 then util_draw.draw_hitboxes(_obj.pos_x, _obj.pos_y, _obj.flip_x, _obj.boxes) end end
        end
        ...
        if spectator_settings.display_hitboxes == 2 or spectator_settings.display_hitboxes == 4 then
            ...
            -- projectiles
            for _id, _obj in pairs(projectiles) do if _obj.emitter_id == 2 then util_draw.draw_hitboxes(_obj.pos_x, _obj.pos_y, _obj.flip_x, _obj.boxes) end end
        end
        ...
        if spectator_settings.display_distances then display_draw_distances(player_objects[1], player_objects[2], 70, 2, 2) end
    end
...
function hotkey1() hotkey1_pressed = true end

function hotkey2() hotkey2_pressed = true end

function hotkey3() hotkey3_pressed = true end

function hotkey4() hotkey4_pressed = true end
