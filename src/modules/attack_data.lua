...
local function attack_data_update(attacker, defender)
    ...
    if attacker.combo == 0 then attack_data.last_hit_combo = 0 end
    ...
    if attacker.combo ~= 0 then attack_data.combo = attacker.combo end
    if attacker.combo > attack_data.max_combo then attack_data.max_combo = attacker.combo end
end
...
local function attack_data_reset() attack_data = {player_id = nil, last_hit_combo = 0, damage = 0, stun = 0, combo = 0, total_damage = 0, total_stun = 0, max_combo = 0} end

attack_data_reset()

return {attack_data = attack_data, attack_data_update = attack_data_update, attack_data_display = attack_data_display, attack_data_reset = attack_data_reset}
